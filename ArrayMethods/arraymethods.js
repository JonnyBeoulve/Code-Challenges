...
spaceXFlights.forEach(flight =>
  console.log(`${flight.flight_number}. ${flight.mission_name}`)
);

// Map: Create a new aray containing only the mission patch URLs for each flight
const flightMissionPatches = spaceXFlights.map(flight => {
  return flight.mission_patch;
});
...
console.log(
  "\nDoes Trailblazer exist in the array? " +
    spaceXFlights.some(flight => flight.mission_name === "Trailblazer")
);

//Some: Output whether or not every flight has a flight number
console.log(
  "\nDoes every flight have a flight number? " +
    spaceXFlights.every(flight => flight.flight_number)
);
